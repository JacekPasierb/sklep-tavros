...
const isDisabledTab = (t: ShopGender) => DISABLED_TABS.includes(t);
...
const MobileMenu = ({ open, onClose }: Props) => {
  ...
  // ✅ POBIERANIE KOLEKCJI Z API (tylko gdy menu otwarte + tylko mens)
  const shouldFetchCollections = open && tab === "mens";

  const { data, isLoading, error } = useSWR(
    shouldFetchCollections ? [`/api/collections`, tab] : null,
          ...
          <div className="flex-none border-b px-4 pt-3 pb-2">
            <div className="mb-2 flex items-center justify-between">
              <div className="flex gap-5 text-sm font-semibold uppercase tracking-wide">
                {TABS.map(({ id, label }) => {
                  const disabled = isDisabledTab(id);
